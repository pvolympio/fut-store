...
const selecoes = times.filter((t) => t.tipo === "selecao");

export function generateStaticParams() {
  return selecoes.map((t) => ({ slug: t.slug }));
}
...
export default function SelecaoPage({ params }: { params: { slug: string } }) {
  const time = getTimePorSlug(params.slug);
  if (!time || time.tipo !== "selecao") notFound();
